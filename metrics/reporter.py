...
class Reporter:
    ...
    def skipBreakdown(cls, skips):
        ...
        if getattr(skips, "noMidPosGuard", 0):
            cls._out(f"  - Mid-pos guard skips : {skips.noMidPosGuard}")
        if getattr(skips, "noWeakBucketGuard", 0):
            cls._out(f"  - Weak-bucket guard skips : {skips.noWeakBucketGuard}")
        if getattr(skips, "noQualityGate", 0):
            cls._out(f"  - Quality-gate skips : {skips.noQualityGate}")
